...
class HRNetV2(nn.Module):
    """
    HRNetV2 支持 freeze_mode：
        - "none": 全部训练
        - "partial": 冻结 stem + layer1 + transition1
        - "full_backbone": 冻结 stem ~ stage2，训练 last_layer
        - "all": 全部冻结（只推理）
    """
    def __init__(self, num_classes=1, in_channels=3, freeze_mode="none"):
    ...
    def apply_freeze(self, mode="none"):
        ...
        if mode == "none":
            ...
            freeze(self.stem, True)
            freeze(self.layer1, True)
            freeze(self.transition1, True)
            freeze(self.stage2, False)
            freeze(self.last_layer, False)
        elif mode == "full_backbone":
            freeze(self.stem, True)
            freeze(self.layer1, True)
            freeze(self.transition1, True)
            freeze(self.stage2, True)
            freeze(self.last_layer, False)
        elif mode == "all":
            freeze(self, True)
